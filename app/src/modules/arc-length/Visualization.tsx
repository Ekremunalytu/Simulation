...
import type { ArcLengthParams, ArcLengthResult } from './logic'

export function ArcLengthVisualization({
  result,
  runtime,
}: VisualizationProps<ArcLengthParams, ArcLengthResult>) {
  const activeFrame =
    result.frames[Math.min(runtime.frameIndex, result.frames.length - 1)] ?? result.frames[0]
  const convergenceData = result.frames.map((frame) => ({
    segmentCount: frame.segmentCount,
    length: frame.approximateLength,
    error: frame.error,
  }))
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-mono uppercase tracking-widest text-outline">
            kırık çizgi yakınsaması
          </span>
        </div>
        <div className="flex gap-6">
          <div className="text-right">
            <p className="text-xs font-mono text-outline uppercase">Parça</p>
            <p className="font-mono text-sm text-primary">{activeFrame.segmentCount}</p>
          </div>
          <div className="text-right">
            <p className="text-xs font-mono text-outline uppercase">Uzunluk</p>
            <p className="font-mono text-sm text-secondary">{activeFrame.approximateLength.toFixed(4)}</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={result.curve}>
                ...
                <YAxis dataKey="y" type="number" stroke="#5a5567" tick={{ fontSize: 12, fill: '#b9b4c8' }} tickLine={false} />
                <Tooltip
                ...
                />
                <Line dataKey="y" type="monotone" stroke="#d0bcff" strokeWidth={2.4} dot={false} />
                <Line data={activeFrame.polyline} dataKey="y" type="linear" stroke="#4cd7f6" strokeWidth={2.1} dot={false} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={convergenceData}>
                <CartesianGrid stroke="#343242" strokeDasharray="3 3" />
                <XAxis dataKey="segmentCount" stroke="#5a5567" tick={{ fontSize: 12, fill: '#b9b4c8' }} tickLine={false} />
